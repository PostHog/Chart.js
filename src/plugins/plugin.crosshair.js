...
module.exports = {
	...
	afterEvent: function (chart, e) {
		...
		var suppressTooltips = this.getOption(
			chart,
			"sync",
			"suppressTooltips"
		);
	...
	beforeTooltipDraw: function (chart) {
		...
		return (
			!chart.crosshair.dragStarted && !chart.crosshair.suppressTooltips
		);
	},
	...
	resetZoom: function (chart) {
		...
		if (chart.crosshair.button && chart.crosshair.button.parentNode) {
			chart.crosshair.button.parentNode.removeChild(
				chart.crosshair.button
			);
	...
	drawTraceLine: function (chart) {
		...
		var isHoverIntersectOff =
			chart.config.options.hover.intersect === false;

		if (snapEnabled && isHoverIntersectOff) {
			if (chart.active.length) {
				lineX = chart.active[0]._view.x;
			} else {
				var xScale = this.getXScale(chart);
				lineX = xScale.getPixelForValue(xScale.getValueForPixel(lineX));
			}
		}
